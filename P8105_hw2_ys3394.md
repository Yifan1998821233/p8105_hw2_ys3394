...
``` r
month_df =  # create a month tibble, and left_join original pre_df data
  tibble(
    month = 1:12,
...
csv file with the readxl package, select required columns, and converted
the entry variable from the character variable to the logical variable.
It can bee seen from the tibble bellow, which is head rows of `NYC_df`.
The dataset is a 1868 X 19 tibble. The dataset is not yet tidy, there
are variables such as those inconsistent route numbers and names, can be
modified.
...
**Summary**
  ...
  - In dataset pols\_df, there is a 1644 X 10 tibble, with the year
    range from 1947 to 2015. Key variables in this dataframe are
    ***president, sen\_dem***, etc.
  ...
  - In the combined dataset, three tibbles that sharing variable `year`
    and `montn_name` are combine together by `left_join`. there are some
    key variables ***year, month\_name, gov\_gop, sen\_gop, rep\_gop,
    gov\_dem, sen\_dem, rep\_dem, president, president\_val, day, close,
    unemployment\_rate*** . The combined dataset has 1644 rows with 13
    columns. The range of the dataset is 68 years, from 1947 to 2015.
